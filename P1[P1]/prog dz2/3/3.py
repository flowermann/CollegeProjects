...
def company_stats(games):
    ...
    for publisher in publishers:
        filtered_games = []
        for game in games:
            if game[4] == publisher and game[10]>=min_rating:
                filtered_games.append([game[0],game[10]])
...
if len(file_data) != 0:
    platforms = platform_sales(file_data)
    write_console_stats = console_stats(file_data)
    write_company_stats = company_stats(file_data)